...
For the example we will look at the beautiful town of penmarc'h in France and we are going to get these borders.
...
```{r}
penmarch_point = st_sfc(st_point(c(-4.370725371956444,47.80004378909002)), crs = 4326)
...
tm_shape(penmarch_point)+ # Point use to retrieve data
   tm_dots(col = "red", size = 2)+
      tm_add_legend(type = "symbol", label = "lat : -4.370, long : 47.800",
                    col = "red", size = 1)+
   tm_layout(main.title = "Penmarc'h borders from IGN",
...
Now you have to rely on your curiosity to explore the multiple possibilities that IGN offers. For example, who has never wondered how many road junctions there are in Penmarc'h?
...
tm_shape(dikes)+ # Point use to retrieve data
   ...
   tm_layout(main.title = "Road nodes recorded by the IGN in Penmarc'h",
             main.title.position = "center",
             legend.position = c("right", "bottom"),
             frame = FALSE)
```
 ...
 * *The raster is not loaded into R's memory. It is an on-the-fly sequential reading of files from network, without downloading the whole file first. In other words, it is quite possible to access the DTM of an entire region. The counterpart is the connection time which is about 30s;*
 * *Raster from `get_wms_raster()` are stars object from the `stars` package. To learn more about conversion betwwen other raster type in R go [check this out](https://geocompr.github.io/post/2021/spatial-classes-conversion/).*
