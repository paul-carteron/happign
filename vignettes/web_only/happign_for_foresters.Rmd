...
```{r borders2}
camors_border2 <- get_wfs(layer = "LIMITES_ADMINISTRATIVES_EXPRESS.LATEST:commune",
...
mnt <- get_wms_raster(camors_border, mnt_layer, res = 5, crs = 2154, rgb = FALSE)
mns <- get_wms_raster(camors_border, mns_layer, res = 5, crs = 2154, rgb = FALSE)

# Add level curve for flexing
level_curve <- get_wfs(camors_border, "ELEVATION.CONTOUR.LINE:courbe",
                       spatial_filter = "intersects") |> 
...
tm_shape(level_curve)+
   tm_lines(col="grey25", col_alpha = 0.8)+
tm_shape(camors_border)+
...
tm_shape(ndvi)+
   tm_raster(
      col.scale = tm_scale_continuous(
         limits = c(-1, 1),
         values = palette_ndvi#, values.repeat = F, value.na = NULL
      ),
...
```{r etat_major}
etat_major <- get_wms_raster(x = camors_border,
                             res = 1,
