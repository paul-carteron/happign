...
# Context
To ensure a viable service, the APIs accessible with happign are tested for errors every 12:00 AM on sonday. If you also detect non-viable APIs, please describe your problem [here](https://github.com/paul-carteron/happign/issues).
...
Testing all **`r nrow(all_metadata)`** WFS ressources took **`r round(as.numeric(time, units="mins"),2)`** mins.
...
Testing all **`r nrow(ressources_to_test)`** WMS ressources for http errors took **`r round(as.numeric(http_time, units="mins"),2)`** mins.
...
Testing all **`r nrow(ressources_to_test)`** WFS ressources took **`r round(as.numeric(vsi_time, units="mins"),2)`** mins.

There are **`r length(no_ressources)` ** categories that have no WMS raster resources : **`r paste0(no_ressources, collapse  =", ")` **.

There are **`r nrow(vsi_error)`** ressources that can't be read by `happign` for the moment. Details of them are displayed below :
