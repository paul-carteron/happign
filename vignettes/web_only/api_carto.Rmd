...
### RNU
First of all, you can check if a commune is under the National Urbanism Regulation from is insee code. The RNU fully apply in the communes that have neither a local map nor a local urban plan (PLU, PLUi) nor a document in replacement of a PLU.
...
```{r}
# find out if documents are available from geometry intersection
penmarch <- get_apicarto_cadastre("29158", "commune")
doc <- get_apicarto_gpu(penmarch, "document", dTolerance = 5) # complex geometry handle with dTolerance
...
doc <- get_apicarto_gpu(lanildut, "document",  dTolerance = 5) # complex geometry handle with dTolerance
...
tm_shape(prescriptions[[1]])+
   tm_polygons("libelle",  legend.show = FALSE)+
...
```{r}
viticole <- get_apicarto_viticole(penmarch, dTolerance = 5)
